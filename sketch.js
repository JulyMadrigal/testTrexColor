...
function setup() {
  ...
  //ideias da minha cabeça
  //quanto mais tempo a pessoa fica no jogo, mais o sol sobe e diminue a escala, a gente pode trabalhar com o placar
  sol = createSprite(width-50,height+100,10,10);
  sol.addAnimation("sun", animacaoDeSol);
  //sol.y = height +100
  //sol.velocityY = -(1 + 1*pontuacao/7);
...
function draw() {
  ...
  if (estadoJogo === JOGAR){
    ...
    solo.velocityX = -(6 + 3*pontuacao/1000);
    ...
    pontuacao = pontuacao + Math.round(getFrameRate()/60);
     ...
     sol.velocityY = -(0.6 + pontuacao/10000000000);
     sol.scale = sol.scale - (pontuacao/1000000);
    if(sol.scale <= 0.1){
      sol.scale =0.1;
    }
    if(sol.y<= height/2 - 250){
      sol.y = height/2 - 250;
    }
...
function gerarObstaculos(){
 if (frameCount % 60 === 0){
   var obstaculo = createSprite(width,height-95,20,30);
    ...
    obstaculo.velocityX = -(6 + 3*pontuacao/1000);
